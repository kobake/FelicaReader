...
#define SAMPLE_TITLE		_T("DEP-Target (Target)")
...
int main(int argc, _TCHAR* argv[])
{
	...
	SENDCOMM	SendComm[] = {
					{11, {0xC4, 0x10, 0x01, 0x10, 0x27, 0x00, 0x00, 0x01, 0x02, 0x03, 0x04}}, /* Start Target */
					{6,  {0xC4, 0x14, 0xE8, 0x03, 0x00, 0x00}},				/* Receive DEP data */
					{51, {0xC4, 0x15, 0x02,
						  ...
						  0x28, 0x29, 0x2A, 0x2B, 0x2C, 0x2D, 0x2E, 0x2F}},	/* Send DEP data */
					{67, {0xC4, 0x15, 0x03,
						  ...
						  0x68, 0x69, 0x6A, 0x6B, 0x6C, 0x6D, 0x6E, 0x6F}},	/* Send DEP data (Chainig) */
					{83, {0xC4, 0x15, 0x01,
						  ...
						  0xB8, 0xB9, 0xBA, 0xBB, 0xBC, 0xBD, 0xBE, 0xBF}},	/* Send DEP data (Chainig) */
					{6,  {0xC4, 0x12, 0x01, 0x02, 0x03, 0x04}},				/* Stop Target */
					{-1, NULL},
				};
	BYTE		bRecvBuf[PCSC_RECV_BUFF_LEN] = {0x00};
	DWORD		dwOutbuffSize = sizeof(bRecvBuf);
	DWORD		dwResponseSize;
	for (UINT uiCmdIdx = 0; SendComm[uiCmdIdx].iLength > -1; uiCmdIdx ++) {
		dwResponseSize = 0;
		lResult = ::SCardControl(hCard, SCARD_CTL_CODE(3500),
			SendComm[uiCmdIdx].bCommand, SendComm[uiCmdIdx].iLength, bRecvBuf, dwOutbuffSize, &dwResponseSize);
		if (lResult != SCARD_S_SUCCESS) {

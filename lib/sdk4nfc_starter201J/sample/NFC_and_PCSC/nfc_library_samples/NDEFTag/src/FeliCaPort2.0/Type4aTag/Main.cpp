...
#define COMMAND_NAME TEXT("Type4aTag")
...
void usage(void)
{
	...
	_tprintf_s(TEXT("  %s\n"), CMD_READ);
	_tprintf_s(TEXT("\t Read from the Type4 Tag.\n"));
	_tprintf_s(TEXT("  %s\n"), CMD_WRITE);
	_tprintf_s(TEXT("\t Write to the Type4 Tag.\n"));
	_tprintf_s(TEXT("\n"));
...
int _tmain(int argc, TCHAR** argv)
{
	...
	try {
		...
		NfcAccessLib lib(NfcAccessLib::TypeA);
		ISO7816_4 tag(&lib);
		Type4TagAccessor accessor(&tag);
		SmartPosterRecord record;
		unsigned char aBuf[0xFFFF];
		unsigned short usLen = sizeof(aBuf);
		if (param.isReadMode) {
			accessor.read(aBuf, &usLen);
			if (param.isDumpMode) {
				printBytes(aBuf, usLen);
			}
			record.setData(aBuf, usLen);
			_tprintf_s(TEXT("TITLE:%s\n"), record.getTitle());
			_tprintf_s(TEXT("URI:  %s\n"), record.getUri());
		} else {
			record.setData("ja", param.pszTitle, param.pszUri);
			record.getData(aBuf, usLen);
			usLen = record.getDataLength();
			accessor.write(aBuf, usLen);
